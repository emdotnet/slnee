...
def execute(filters=None):
	...
	for parent_item in parent_items:
		...
		i=0
		total=0
		last=len(warehouse_company_map)
		qty={}
		for warehouse, company in iteritems(warehouse_company_map):
			...
			for child_item_detail in required_items:
				...
				if child_item_detail.item_code in qty.keys():
					try:
						qty[child_item_detail.item_code]+=(child_item_balance.qty_after_transaction)
					except:
						a=1
				else:
					qty[child_item_detail.item_code]=flt(child_item_balance.qty_after_transaction)
			min_bundle_qty = min(map(lambda d: d["bundle_qty"], child_rows))
			parent_row["bundle_qty"] = min_bundle_qty
			total+=min_bundle_qty
			if old_childs == []:
				old_childs = child_rows
			else:
				for h in range(len(old_childs)):
					old_childs[h]["actual_qty"]+=child_rows[h]["actual_qty"]
			i+=1
			if group:
				parent_row["bundle_qty"]=total
				if i==last:
					for r in old_childs:
						item=r['item_code']
						r["actual_qty"]=qty[item]
						try:
							r["bundle_qty"] = r["actual_qty"] // r["minimum_qty"]
						except:
								r["bundle_qty"]=-1
								frappe.throw(str(qty))
					min_bundle_qty = min(map(lambda d: d["bundle_qty"], old_childs))
					parent_row["bundle_qty"]=min_bundle_qty
