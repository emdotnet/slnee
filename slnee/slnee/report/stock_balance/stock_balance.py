...
def execute(filters=None):
	...
	#frappe.throw(str(iwb_map))
	itemss=frappe.db.get_list("Item",fields=["name","image"])
	images={}
	for i in itemss:
	...
	for (company, item, warehouse) in sorted(iwb_map):
		if item_map.get(item):
			...
			if images[item]:
				report_data["item_image"]="<span><img style='max-height:110px;width:auto !important;' src='"+images[item]+"'></span>"
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql(
		"""
		select
			item.name,item.image, item.item_name, item.description, item.item_group, item.brand, item.stock_uom %s
		from
			`tabItem` item
			%s
		where
